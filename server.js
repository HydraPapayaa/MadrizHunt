...
const app = express();
const dbFile = path.resolve(__dirname, 'madriz.db');
const db = new sqlite3.Database(dbFile, sqlite3.OPEN_READWRITE | sqlite3.OPEN_CREATE, (err) => {
  if (err) {
    console.error('No se pudo abrir la base de datos SQLite en', dbFile, err);
    process.exit(1);
  }
  console.log('Base de datos SQLite abierta en', dbFile);
});
...
app.post('/api/users/change-password', requireAuth, (req, res) => {
  ...
  db.get('SELECT password FROM users WHERE email = ?', [cur.email], async (err, row) => {
    if (err || !row) return res.status(500).json({ error: 'DB error' });
    const match = await bcrypt.compare(currentPassword, row.password);
    if (!match) return res.status(400).json({ error: 'Contraseña actual incorrecta' });
    const hashed = await bcrypt.hash(newPassword, 10);
    db.run('UPDATE users SET password = ? WHERE email = ?', [hashed, cur.email], function(err) {
      if (err) return res.status(500).json({ error: 'DB error' });
      logAction(cur.email, 'change_password', 'Contraseña actualizada');
      res.json({ ok: true });
    });
  });
});
...
app.get('/api/map/points', (req, res) => {
  db.all('SELECT id, title, location, description, photo_url, lat, lng, created_by FROM map_points ORDER BY created_at DESC', [], (err, rows) => {
    if (err) return res.status(500).json({ error: 'DB error' });
...
app.post('/api/map/points', requireAuth, (req, res) => {
  const cur = req.session.user;
  if (!(cur.isHydra || cur.role === 'owner' || cur.role === 'staff')) return res.status(403).json({ error: 'No tienes permisos para crear puntos' });

  const { title, location, description, photoUrl, lat, lng } = req.body;
  if (!title || !description || typeof lat !== 'number' || typeof lng !== 'number') return res.status(400).json({ error: 'Faltan campos obligatorios' });

  db.run('INSERT INTO map_points (title, location, description, photo_url, lat, lng, created_by) VALUES (?, ?, ?, ?, ?, ?, ?)',
    [title, location || '', description, photoUrl || '', lat, lng, cur.name || cur.email], function(err) {
      if (err) return res.status(500).json({ error: 'DB error' });
      logAction(cur.email, 'create_map_point', `Creó punto ${title}`);
      res.json({ ok: true, pointId: this.lastID });
    });
});
